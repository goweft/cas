"""CAS API router — extends Heddle's FastAPI app with workspace routes."""
...
@router.post("/session")
def new_session():
    """Create a fresh session explicitly. Returns the new session id."""
    session = _shell.create_session()
...
def send_message_stream(req: MessageRequest):
    """Stream via SSE. Events: session, intent, token×N, workspace?, chat_reply, done."""
    session_id, session = _resolve_session(req.session_id)
    message = req.message

    from cas.shell import detect_intent
    ...
    def generate():
        ...
        if intent.kind == "create_workspace":
            ...
            _shell._auditor.log_workspace_create(ws, session_id)
            ...
            if not active:
                ...
                _shell._auditor.log_workspace_update(ws, session_id, message)
                _shell._conductor.observe(intent.kind, message, workspace_title=ws.title, ws_type=ws.type)
...
def get_workspace_history(workspace_id: str):
    ...
    return [{"version": h["version"], "title": h["title"],
